...
namespace WebApiCoursesService.Controllers
{
    public class NckuController : ApiController
    {
        ...
        public IEnumerable<NckuCourseModel> GetAll()
        {
        ...
        public async void Post([FromBody]NckuCourseModel coursedata)
        {
            var _client = new MongoClient();
            var _database = _client.GetDatabase("SuperUniversityCourses");
            var collection = _database.GetCollection<NckuCourseModel>("nckuTest");
        ...
        //// PUT: api/Ncku/5(Update)
        public async void PutComment(string strid, [FromBody]Comment comment)
        {
            //id 前面加上[FromBody]，表示要從 HTTP POST 內文來取得參數值：
            var _client = new MongoClient();
            var _database = _client.GetDatabase("SuperUniversityCourses");
            ...
            var filter = Builders<NckuCourseModel>.Filter.Eq("_id", Objid);
            var cursor = collection.Find(filter);
            List<Comment> OrigianlComments = cursor.FirstOrDefault().commentdata;
        ...
        public async void PutRanking(string strid, int sweet,int relaxing, int deep)
        {
            ...
            var filter = Builders<NckuCourseModel>.Filter.Eq("_id", Objid);
            var cursor = collection.Find(filter);
            List<Ranking> OrigianlRanking = cursor.FirstOrDefault().rankingdata;
            Ranking InputRanking = ranking;


            UpdateDefinition<NckuCourseModel> update;
            if (OrigianlRanking == null)
            {
                List<Ranking> InputRankingData = new List<Ranking>();
                InputRankingData.Add(InputRanking);
                update = Builders<NckuCourseModel>.Update
            ...
            {
                OrigianlRanking.Add(InputRanking);
                update = Builders<NckuCourseModel>.Update
                            .Set("rankingdata", OrigianlRanking)
                            .CurrentDate("lastModified");
            }
